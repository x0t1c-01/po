...
int AnalizzatoreCerchi::metodo1()
{
    if(cerchi.empty())
    	return -1;
	int isolati=0;
	bool disconnesso=true;
	int x,y,r;
	for(list<Cerchio>::iterator it=cerchi.begin();it!=cerchi.end();it++){
		for(list<Cerchio>::iterator it1=cerchi.begin();it1!=cerchi.end();it1++){
			if( it!=it1 ){
				x= ( (it->getX()) - (it1->getX()) ) * ( (it->getX()) - (it1->getX()) );
				y= ( (it->getY()) - (it1->getY()) ) * ( (it->getY()) - (it1->getY()) );
				r= ( (it->getRaggio()) + (it1->getRaggio()) ) * ( (it->getRaggio()) + (it1->getRaggio()) );
				if( (x+y)<=r )
					disconnesso=false;
			}
		}
...
int AnalizzatoreCerchi::metodo3()
{
    if(cerchi.empty())
   		 return -1;
	vector<int> raggi;
	vector<Cerchio> distinti;
	int min=INT_MAX;
	int min1=INT_MAX;
	for(list<Cerchio>::iterator it=cerchi.begin();it!=cerchi.end();it++){
		for(list<Cerchio>::iterator it1=cerchi.begin();it1!=cerchi.end();it1++){
			if( it!=it1 ){
				if( (it->getRaggio())==(it1->getRaggio()) && find(distinti.begin(),distinti.end(),(*it1))==distinti.end() ){
					distinti.push_back((*it1));
					raggi.push_back((it->getRaggio()));
				}
			}
		}
	}
	if(raggi.size()<2)
		return 0;
	for(int i=0;i<raggi.size();i++){
		if(raggi[i]<min)
			min=raggi[i];
	}
	for(int i=0;i<raggi.size();i++){
		if(raggi[i]<min1 && raggi[i]>min)
			min1=raggi[i];
	}
	//cout<<min1<<min<<endl;
	return min1-min;
...
int AnalizzatoreCerchi::metodo4()
{
	...
	int g;
	Cerchio A;
	vector<Cerchio> positivi;
	for(list<Cerchio>::iterator it=cerchi.begin();it!=cerchi.end();it++){
		if( (it->getX()>0) && (it->getY()>0) )
			positivi.push_back( (*it) );
	}
	if(positivi.empty())
		return -1;
	A=positivi.size()-1;
	...
	for(int i=0;i<positivi.size();i++){
		g=mcd(positivi[i].getX(),positivi[i].getY());
		if(g<M)
			cont++;	
	}
	if(cont==0)
