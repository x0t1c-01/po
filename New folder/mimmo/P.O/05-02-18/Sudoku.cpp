...
int Sudoku::metodo1()
{
	...
	for(list<Cella>::iterator it=sudoku.begin();it!=sudoku.end();it++){
		if( find(righe.begin(),righe.end(),(it->getRiga()))==righe.end() )
			righe.push_back( (it->getRiga()) );
	}
	for(int i=0;i<righe.size();i++){
		for(list<Cella>::iterator it=sudoku.begin();it!=sudoku.end()&&!trovato;it++){
			if((it->getRiga())==righe[i]){
				for(list<Cella>::iterator it1=sudoku.begin();it1!=sudoku.end()&&!trovato;it1++){
					if((it1->getRiga())==righe[i] && (it1->getColonna())!=(it->getColonna()) && (it->getSimbolo())==(it1->getSimbolo())){
						cont++;
						trovato=true;
					}
				}
...
int Sudoku::metodo2()
{
	...
	int cont=0;
	for(list<Cella>::iterator it=sudoku.begin();it!=sudoku.end();it++){
		mat[it->getRiga()][it->getColonna()]=(it->getSimbolo());
	}
	for(int i=0;i<9;i++){
		for(int j=0;j<9;j++){
			if(mat[i][j]==0)
